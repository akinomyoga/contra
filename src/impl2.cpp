...
void write_loop(int fddst, char* buff, ssize_t nread, int milliseconds) {
  ssize_t nrest = nread;
  const char* p = buff;
  for (; nrest > 0; ) {
...
bool process_controls(int fdsrc, int fddst, char* buff, std::size_t size, int milliseconds, contra::tty_player& term) {
  ssize_t const nread = read(fdsrc, buff, size);
  if (nread <= 0) return false;

  term.write(buff, nread);
  write_loop(fddst, buff, nread, milliseconds);
  return true;
}
...
bool transfer_stream_data(int fdsrc, int fddst, char* buff, std::size_t size, int milliseconds) {
  ssize_t const nread = read(fdsrc, buff, size);
  if (nread <= 0) return false;
...
int main() {
  ...
  if (pid == 0) {
    ...
    char buff[4096];
    for (;;) {
      if (process_controls(masterfd, STDOUT_FILENO, buff, sizeof(buff), 1, term)) continue;
